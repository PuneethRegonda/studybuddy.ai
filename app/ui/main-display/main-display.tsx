...
import MiniGameContent from './content/mini-game-content'; // ✅ Import

import {
  SAMPLE_FLIP_CARD_DATA,
  SAMPLE_MIND_MAP_DATA,
  SAMPLE_MINI_GAME,
  SAMPLE_QUIZ_DATA,
  SAMPLE_TEXT_DATA,
} from '@/app/lib/sample-data';
import { Spinner } from '../dashboard/redirect';
import MindmapContent from './content/mindmap-content'; // import it
...
export default function MainDisplay({
  ...
  const renderContent = () => {
    let contentTypeToRender = selectedContentType;
    let contentDataToRender = null;
  
    if (demoMode) {
      // In Demo Mode, use Sample Data
      switch (contentTypeToRender) {
        case 'text':
          contentDataToRender = SAMPLE_TEXT_DATA.data;
          return <TextContent data={contentDataToRender} />;
        case 'flipcard':
          contentDataToRender = SAMPLE_FLIP_CARD_DATA.data;
          return <FlipCardContent data={contentDataToRender} />;
          case 'mindmap':
              contentDataToRender = SAMPLE_MIND_MAP_DATA.data;
            return <MindmapContent data={contentDataToRender} />;
        case 'quiz':
          contentDataToRender = SAMPLE_QUIZ_DATA.data;
          return <QuizContent data={contentDataToRender} />;
        case 'react':
          contentDataToRender = SAMPLE_MINI_GAME.data
          return   <MiniGameContent data={contentDataToRender} />;
        default:
        ...
        {demoMode && (
          <div className="flex flex-wrap justify-center gap-4 mt-2">
            ...
            <button
              ...
              className={`flex items-center px-4 py-2 rounded-full border transition ${
                selectedContentType === 'mindmap'
                  ? 'bg-blue-100 border-blue-600 text-blue-600'
                  : 'bg-white border-gray-300 text-gray-700'
              }`}
            >
              <span className="mr-2">💭</span> Mind Map
            </button>
