...
export default function AttentionLevelTracker({
  ...
  const getContentTypeForAttentionLevel = (level: number): string => {
    ...
    if (level > 60) return 'mindmap';
    if (level > 40) return 'flipcard';
  ...
  useEffect(() => {
    ...
    const shouldSuggestBreak = (consecutiveSameRangeRef.current >= 3) && (avg < 50);
  ...
  const handleContentChange = async () => {
    ...
    if (isSwitchingContent) {
      console.log("Already switching, skip...");
      return;
    }
    ...
    try {
      ...
      if (onAttentionChange && isContentLoaded) {
        ...
        if (suggestedContentType === 'flipcard') {
          generatedContent = await generateFlashcards(currentSummaryText);
        } else if (suggestedContentType === 'quiz') {
          generatedContent = await generateQuiz(currentSummaryText);
        } else if (suggestedContentType === 'mindmap') {
          generatedContent = await generateMindmap(currentSummaryText);
        } else if (suggestedContentType === 'react') {
          generatedContent = await generateMiniGame(currentSummaryText);
        } else {
          // Default to text (summarized text you already have)
          generatedContent = {
            id: 'text-content',
            type: 'text',
            data: { title: 'Summary', content: currentSummaryText }
          };
        }
      ...
      console.error('Error switching content dynamically:', err);
    }finally {
      setTimeout(() => {
        setIsSwitchingContent(false);
      }, 10000);  // Wait 10 seconds before allowing next switch
    }
  };
